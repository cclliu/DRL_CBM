...
def save_records(env, episode, save_dir, episode_losses, episode_rewards):
    ...
    with open(state_records_path, "w") as f:
        for i, state in enumerate(env.state_records[:max_time_steps]):  # 只保存前55个时间步
            f.write(f"时间步 {i}:\n{state}\n\n")

    # 保存动作记录
    action_records_path = os.path.join(save_dir, f"action_records_episode_{episode}.txt")
    with open(action_records_path, "w") as f:
        for i, action in enumerate(env.action_records[:max_time_steps]):  # 只保存前55个时间步
    ...
    with open(reward_records_path, "w") as f:
        for i, reward in enumerate(env.reward_records_detailed[:max_time_steps]):  # 只保存前55个时间步
    ...
    with open(gas_production_records_path, "w") as f:
        for i, gas in enumerate(env.gas_production_records[:max_time_steps]):  # 只保存前55个时间步
            f.write(f"时间步 {i}: {gas}\n")
    ...
    with open(drainage_records_path, "w") as f:
        for i, drainage in enumerate(env.drainage_records[:max_time_steps]):  # 只保存前55个时间步
    ...
    with pd.ExcelWriter(excel_path) as writer:
        ...
        if len(env.state_records) > 0:
            state_records_flat = np.array(env.state_records[:max_time_steps]).reshape(len(env.state_records[:max_time_steps]), -1)
            pd.DataFrame(state_records_flat).to_excel(writer, sheet_name="状态记录", index=False)

        # 保存动作记录
        if len(env.action_records) > 0:
            pd.DataFrame(env.action_records[:max_time_steps]).to_excel(writer, sheet_name="动作记录", index=False)

        # 保存奖励记录
        if len(env.reward_records_detailed) > 0:
            pd.DataFrame(env.reward_records_detailed[:max_time_steps], columns=["奖励"]).to_excel(writer, sheet_name="奖励记录", index=False)

        # 保存天然气产量记录
        if len(env.gas_production_records) > 0:
            pd.DataFrame(env.gas_production_records[:max_time_steps]).to_excel(writer, sheet_name="天然气产量记录", index=False)

        # 保存排水量记录
        if len(env.drainage_records) > 0:
            pd.DataFrame(env.drainage_records[:max_time_steps]).to_excel(writer, sheet_name="排水量记录", index=False)
...
def train():
    ...
    for e in range(episodes):
        ...
        if e % 200 == 0:
            model_save_path = os.path.join(current_dir, "weights", f"dqn_gas_extraction_model_episode_r1_{e}.pth")
    ...
    plt.savefig(reward_plot_path, bbox_inches='tight',dpi=300)
    plt.close()  # 关闭图像，避免内存泄漏
