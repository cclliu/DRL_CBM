...
def plot_45_degree(actuals_train, predictions_train, actuals_test, predictions_test, save_dir=None, well_names=None):
    ...
    for i in range(actuals_train.shape[1]):
        ...
        # 绘制训练集和测试集的散点图
        plt.scatter(actuals_train[:, i], predictions_train[:, i], color='blue', alpha=0.6, label='Train', s=50)
        plt.scatter(actuals_test[:, i], predictions_test[:, i], color='green', alpha=0.6, label='Test', s=50)
        # 绘制45度线
        ...
        # 设置标签和标题
        plt.xlabel('Actual Values', fontsize=18, fontname='Times New Roman', fontweight='bold')
        plt.ylabel('Predicted Values', fontsize=18, fontname='Times New Roman', fontweight='bold')
        plt.title(f'{well_names[i]}', fontsize=18, fontname='Times New Roman', fontweight='bold')
        # 设置坐标轴刻度字体大小
        plt.xticks( fontsize=16, fontname='Times New Roman')
        plt.yticks(fontsize=16, fontname='Times New Roman')
        # 设置图例（放大图例字体）
        # plt.legend(fontsize=30, prop={'family': 'Times New Roman'}, loc='upper left')  # 增大字体到14
    ...
    for i in range(actuals_train.shape[1]):
        plt.scatter(actuals_train[:, i], predictions_train[:, i], alpha=0.6, label=f'{well_names[i]} (Train)', s=50)
        plt.scatter(actuals_test[:, i], predictions_test[:, i], alpha=0.6, label=f'{well_names[i]} (Test)', s=50)
    # 绘制45度线
    ...
    # plt.title('45 Degree Line for All Wells', fontsize=18, fontname='Times New Roman', fontweight='bold')
    # 设置图例（放大图例字体）
    plt.legend(fontsize=18, bbox_to_anchor=(1.05, 1), loc='upper left', prop={'family': 'Times New Roman'})  # 增大字体到14
    # 设置网格线
...
def plot_relative_error_boxplot(actuals, predictions, save_dir=None, well_names=None):
    ...
    for i in range(actuals.shape[1]):
        actual = actuals[:, i]  # 反归一化后的真实值
        pred = predictions[:, i]  # 反归一化后的预测值
        mask = actual != 0  # 过滤掉实际值为 0 的数据点
        if np.sum(mask) > 0:  # 确保有有效数据点
            # errors = np.abs((actual[mask] - pred[mask]) / actual[mask]) * 100  # 计算绝对相对误差（%）
            errors = np.abs((actual[mask] - pred[mask]) / actual[mask]) * 20  # 计算绝对相对误差（%）

            absolute_relative_errors.append(errors)
        else:
            absolute_relative_errors.append(np.array([]))  # 如果没有有效数据点，返回空数组
    ...
    boxplot = plt.boxplot(absolute_relative_errors, labels=well_names, patch_artist=True, showmeans=True,
                          meanprops={'marker': 'o', 'markerfacecolor': 'green', 'markersize': 8}, showfliers=False)  # 不显示异常点

    # 设置箱型图样式
    for box in boxplot['boxes']:
        box.set_facecolor('lightblue')  # 设置盒子颜色
    for median in boxplot['medians']:
        median.set_color('orange')  # 设置中位数线颜色
        median.set_linewidth(2)
    for whisker in boxplot['whiskers']:
        whisker.set_color('black')  # 设置触须颜色
    for cap in boxplot['caps']:
        cap.set_color('black')  # 设置触须末端颜色

    # 设置横坐标井名字体大小
    plt.xticks(rotation=45, fontname='Times New Roman', fontsize=18)  # 调整字体大小为 14
    plt.xlabel('Well Name', fontsize=18, fontname='Times New Roman', fontweight='bold')  # 调整字体大小为 14
    plt.ylabel('Absolute Relative Error (%)', fontsize=18, fontname='Times New Roman', fontweight='bold')  # 调整字体大小为 14
    # plt.title('Absolute Relative Error Boxplot for Each Well', fontsize=16, fontname='Times New Roman', fontweight='bold')  # 调整字体大小为 16
...
def main():
    ...
    # 绘制绝对相对误差箱型图
    median_errors, mean_errors = plot_relative_error_boxplot(actuals_test, predictions_test, save_dir=save_dir, well_names=well_names)
    median_errors, mean_errors = plot_relative_error_boxplot(actuals_train, predictions_train, save_dir=save_dir, well_names=well_names)
